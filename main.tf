...
resource "aws_internet_gateway" "wp_internent_gateway" {
...
resource "aws_route_table" "wp_public_rt" {
  ...
  route {
    ...
    gateway_id = "${aws_internet_gateway.wp_internent_gateway.id}"
  }
...
resource "aws_security_group" "wp_private_sg" {
  name        = "wp_private_sg"
  description = "Used for the elastic load balancer for private"
  vpc_id      = "${aws_vpc.wp_vpc.id}"
...
resource "aws_security_group" "wp_rds_sg" {
  ...
  #SQL access from public/private secirity groups
...
EOD
  ...
  provisioner "local-exec" {
    command = "aws ec2 wait instance-status-ok --instance-ids ${aws_instance.wp_dev.id} --profile bhusan && ansible-playbook -i aws_hosts wordpress.yml"
  }
}
